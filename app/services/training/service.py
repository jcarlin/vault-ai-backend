"""Training job CRUD service — manages DB records for training jobs."""
...
def _row_to_response(row: TrainingJob) -> TrainingJobResponse:
    ...
    lora_config = LoRAConfig(**json.loads(row.lora_config_json)) if row.lora_config_json else None
...
class TrainingService:
    ...
    async def create_job(self, data: TrainingJobCreate) -> TrainingJobResponse:
        ...
        row = TrainingJob(
            ...
            lora_config_json=json.dumps(data.lora_config.model_dump()),
        )
        async with self._session_factory() as session:
            session.add(row)
    ...
    async def update_job_status(
        ...
        *,
        ...
        """Update job fields from the training runner (progress, metrics, error, etc.)."""
        async with self._session_factory() as session:
            result = await session.execute(
                select(TrainingJob).where(TrainingJob.id == job_id)
            )
